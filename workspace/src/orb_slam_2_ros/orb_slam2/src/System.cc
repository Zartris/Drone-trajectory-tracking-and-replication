...
namespace ORB_SLAM2 {
    ...
    nav_msgs::Path System::getGoodKeyFrames(int path_number) {
        // Get all the recorded KFs up to now (and sort them chronologically)
        vector<KeyFrame *> allKFs = mpMap->GetAllKeyFrames();

        sort(allKFs.begin(), allKFs.end(), KeyFrame::lId);

        // Now remove all those that aren't good
        nav_msgs::Path path;
        std::string s;
        s.append(std::to_string(path_number));
        s.append(" <");
...
//            }
            ...
            if (recording == path_number && pKF->mnId > start_record_mnId) {
                pKF->SetRecorded(true, path_number);
            }

            if ((pKF->recorded && pKF->vid_number == path_number)) {
                s.append(std::to_string(pKF->mnId) + ", ");

                cv::Mat R = pKF->GetRotation().t();
                vector<float> q = Converter::toQuaternion(R);
                cv::Mat t = pKF->GetCameraCenter();
                geometry_msgs::PoseStamped pose;
                pose.pose.position.x = t.at<float>(2);
                pose.pose.position.y = -t.at<float>(0);
                pose.pose.position.z = -t.at<float>(1);
    ...
    bool System::getRecordingState() {
        return recording != 0;
    }

    void System::ToggleRecording1() {
        vector<KeyFrame *> allKFs = mpMap->GetAllKeyFrames();
        sort(allKFs.begin(), allKFs.end(), KeyFrame::lId);
        if (!getRecordingState()) {
            start_record_mnId = allKFs[allKFs.size() - 1]->mnId;
            // delete old recording
            for (auto pKF : allKFs) {
                if (pKF->vid_number == 1) {
                    pKF->SetRecorded(false, 0);
                }
            }

        } else {
            end_record_mnId = allKFs[allKFs.size() - 1]->mnId;
            for (auto pKF : allKFs) {

                // remove unwanted path
                if (/**pKF->isBad() ||**/ pKF->mnId < start_record_mnId || pKF->mnId > end_record_mnId) {
                    continue;
                }
                pKF->SetRecorded(true, 1);
    ...
    void System::ToggleRecording2() {
        ...
        if (!getRecordingState()) {
            ...
            for (auto pKF : allKFs) {
                ...
                pKF->SetRecorded(true, 2);
            }
        }
        recording = (recording == 0) ? 2 : 0;
    }
